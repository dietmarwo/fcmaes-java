...
public class CoordRetry {
	...
	/**
	 * Perform a coordinated parallel retry. To be used if the objective function is to be evaluated fast 
	 * but optimization is hard. https://www.esa.int/gsp/ACT/projects/gtop/messenger_full/ can be solved
	 * in about one hour on a modern multi core machine. 
	 * 
	 * @param runs         	number of parallel optimization runs.
	 * @param fit         	Function to optimize. Limits - lower() and upper() - have to be defined.
	 * @param opt         	Optimizer used.
	 * @param guess     	Starting point.
	 * @param limitVal      Maximum value for an optimization result to be stored.
	 * @param startEvals 	Initial	maximum number of evaluations. Will increase incrementally.
	 * @param log      		Flag indicating if the results are to be logged. 
	 */
   public static Optimize optimize(int runs, Fitness fit, Optimizer opt, double[] guess, double limitVal, 
    		int startEvals, boolean log) {
    	Fitness[] store = new Fitness[500];
        Optimize retry = new Optimize(runs, fit, guess, store, limitVal, 
    ...
    public static class Optimize implements Runnable {
    	...
    	/**
    	 * @param runs         	number of parallel optimization runs.
    	 * @param fit0         	Function to optimize. Limits - lower() and upper() - have to be defined.
    	 * @param guess     	Starting point.
        	...
        	if (_log)
	    		synchronized (_store) {
	    			if (_numStored > 0) {
	                    ...
	                    System.out.println(iter + " " + _numStored + " " +
	                    		Utils.r(_evalFac,1) + " " +
	                    		Utils.r(time) + " " + 
	        					_countAll.get() + " " + 
	        				    Utils.r((_countAll.get() + 0.0) / time) + " " + 
	                            Utils.r(best,8) + " " + 
	                            Utils.r(worst) + " " +
	                            statY.toString() + " " + 
	                            storeVals() + " " + 
		...
		public void run() {
			...
			while ((i = _next.getAndIncrement()) < _numRetries) {
				...
				fit.minimize(_opt, _fit0.lower(), _fit0.upper(), _guess, 
						sdev, evalNum(), _popsize);
				addResult(i, fit, _limitVal);
				if (Utils.rnd(1.0/Runtime.getRuntime().availableProcessors()))
					dump(i);
			}
		}
    	...
    	/**
    	 * Crossover of optimization results in the store. 
    	 * Better results have a higher chance to be chosen.
    	 */
	     ...
	     *	Triggers sorting after check_interval calls. 
    	 */
