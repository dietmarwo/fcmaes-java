...
public class Solo extends Fitness {
    ...
    static String resoFile_ = "i3resos0.txt";
...
//        return new double[] {7342.0-dt, 324.0-dt, 110.0-dt, 281.0-dt, -Math.PI, -Math.PI, -Math.PI, -Math.PI, -Math.PI, -Math.PI};
    }
...
//            buf.append(sorted.get(y).prettyDetails() + "\n");
    ...
    void improveTrajectories() {
        ...
        for (double y: sorted.keySet()) {
            ...
            System.out.println("improve " + y + Arrays.toString(tra.x));
            Utils.startTiming();
            fit.minimizeN(128, new Bite(6), tra.x, 120000, 0, 31, 1E99);
    ...
    public double eval(double[] x) {
        ...
        for (int r = 1; r < resos.length; r++) {
            ...
            res = Resonance.resonance(venus, t, v_in, resos_[r], beta[r], 
                    safe_distance, outs, dvs);
        ...
        double value = 100*Utils.sum(dvs) + reso_penalty + 100*(corrected_inclination) + 
                ...
                5000*(Math.max(0, emp_perhelion-min_dist_sun)) +
                0.5*time_val + 50000 * distance_penalty;        
        _usedResos = resos;
        ...
        if (value < bestY_ && value < max_log_y_value) {
             ...
             System.out.println("" + Utils.r(Math.toDegrees(finKep.i())) +  " " + Utils.r(min_sun_distance) + " " + 
                     Utils.r(max_sun_distance) + " " + Utils.r(emp_perhelion) + " " + 
                     Utils.r(Utils.sum(dvs)) + " " + Utils.r(reso_penalty) + " " + Utils.r(time_val) + " " + 
...
//        CoordRetry.optimize(80000, this, new DeBite(), null, 1E99, 0, 2000, true);        
    ...
    void test() {
        double[] x = new double[] {7454.2387984338575, 404.2443240725132, 166.74256469946602, 332.1934804674862, 
                0.1559306807559432, 0.6249560220300024, 0.07091341884521088, 0.49987925340336786, 
                0.09146709709766634, 0.09145589323405663
        };  
...
//      solo.test();
//        solo.join_logs("logs", "reso", "collected.txt", true);
    }
