...
public:
    ...
    MoDeOptimizer(long runid_, Fitness *fitfun_, int dim_, int nobj_, int ncon_,
            int seed_, int popsize_, int maxEvaluations_, double F_, double CR_,
    ...
    vec nextX(int p) {
        if (p == 0) {
            ...
            if (iterations % log_period == 0)
                fitfun->log(popX.cols(), popX.size(), popY.size(), popX.data(),
                        popY.data());
        }
        if (nsga_update) {
            vec x = vX.col(vp);
 ...
 */
JNIEXPORT jdoubleArray JNICALL Java_fcmaes_core_Jni_optimizeMODE(JNIEnv *env,
        jclass cls, jobject func, jint dim, jint nobj, jint ncon,
        jdoubleArray jlower, jdoubleArray jupper, jint maxEvals,
    ...
    MoDeOptimizer opt(runid, &fitfun, dim, nobj, ncon, seed, popsize, maxEvals,
            F, CR, pro_c, dis_c, pro_m, dis_m, nsga_update, pareto_update,
            log_period);
    try {
        if (workers < 1)
            opt.doOptimize();
...
JNIEXPORT jlong JNICALL Java_fcmaes_core_Jni_initMODE(JNIEnv *env, jclass cls,
        jobject func, jint dim, jint nobj, jint ncon, jdoubleArray jlower,
        jdoubleArray jupper, jint maxEvals, jdouble stopfitness, jint popsize,
        jdouble keep, jdouble F, jdouble CR, jdouble pro_c, jdouble dis_c,
        jdouble pro_m, jdouble dis_m, jboolean nsga_update,
        jdouble pareto_update, jint log_period, jlong seed, jint runid) {
    ...
    Fitness *fitfun = new Fitness(callJava, dim, nobj + ncon, lower_limit,
            upper_limit);
    MoDeOptimizer *opt = new MoDeOptimizer(runid, fitfun, dim, nobj, ncon, seed,
            popsize, maxEvals, F, CR, pro_c, dis_c, pro_m, dis_m, nsga_update,
            pareto_update, log_period);
    env->ReleaseDoubleArrayElements(jupper, upper, 0);
    env->ReleaseDoubleArrayElements(jlower, lower, 0);
    return (intptr_t) opt;
}
