...
public:
    CSMAESOpt() :
            ParamCount(0), PopSize(0), PopOrder(NULL), CurParamsBuf(NULL), CurParams(
                    NULL), CurCosts(NULL), MinValues(NULL), MaxValues(NULL), BestParams(
                    NULL) {
    }
    ...
    void init(CBiteRnd &rnd, const double *const InitParams = NULL,
        ...
        for (i = 0; i < ParamCount; i++) {
            CurParams[0][i] =
                    InitParams == NULL ?
                            (MinValues[i] + MaxValues[i]) * 0.5 : InitParams[i];
            CurParams[1][i] = fabs(MaxValues[i] - MinValues[i])
                    * (sdevs == NULL ? 1.0 / 6.0 : sdevs[i]);
        }
